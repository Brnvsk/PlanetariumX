...
const setup = function (numOfSeats: number, radius: number, angles: number[], side: 'left' | 'right', row: number, direction: number) {
    ...
    for (let i = 1; i <= numOfSeats; i++) {
        ...
        const isBooked = bookedSeats.findIndex(booked => {
            return booked.side === side && booked.row === row && booked.place === i
        }) > -1
...
const generate = function(side: 'left' | 'right', row: number, numOfSeats: number, radius: number, direction = 1) {
    ...
    return setup(numOfSeats, radius, angles, side, row, direction)
}
...
let bookedSeats: ISeatOptions[] = []

function getSeatsMap(options: {
    bookedPlaces: IBooking[]
}) {
    const { bookedPlaces } = options
    bookedSeats = []
    bookedPlaces.forEach(booking => {
        const { places } = booking
        bookedSeats.push(...places)
    })
    
    const left1 = generate('left', 3, 17, initialRadius * outerCircleKoeff);
    const left2 = generate('left', 2, 15, initialRadius * middleCircleKoeff);
    const left3 = generate('left', 1, 11, initialRadius * innerCircleKoeff);
    const right1 = generate('right', 3, 17, initialRadius * outerCircleKoeff, -1);
    const right2 = generate('right', 2, 15, initialRadius * middleCircleKoeff, -1);
    const right3 = generate('right', 1, 11, initialRadius * innerCircleKoeff, -1);
